...
extends Building {
    public static final double WIDTH = 75.0;
    ...
    public Node draw() {
        ...
        graphicsGroup.getChildren().addAll(new Node[]{this.nodes.factory});
    ...
    extends Building.NodeHolder {
        private EnhancedGroup factory;

        public NodeHolder() {
            DataLoader loader = DataLoader.getInstance();
            try {
                this.factory = loader.getGameSceneForm(GameSceneModelSource.FACTORY).clone();
                this.factory.setRy(90.0);
                this.factory.setScale(2.0);
            }
            catch (CloneNotSupportedException e) {
                Factory.this.logger.severe("Cloning failed:" + e.getMessage());
                StringWriter errors = new StringWriter();
                e.printStackTrace(new PrintWriter(errors));
                Factory.this.logger.severe(errors.toString());
            }
        }
